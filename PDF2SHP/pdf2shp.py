...
def parse_luas(line):
    """
    Ekstrak nilai luas dan satuan sesuai dokumen (Ha atau m²).
    Mengembalikan (nilai_numeric, satuan_string)
    """
...
def deduplicate_coords(coords, tolerance=1e-6):
    """Hilangkan duplikasi koordinat akibat teks ganda / OCR pada PDF."""
    unique = []
...
def detect_and_transform_coords(coords):
    ...
    Mengembalikan (transformed_coords_list, detected_zone_int_or_None, hemisphere 'N'/'S'/None)

    Logika:
    - Jika sudah lon/lat (nilai dalam rentang Indonesia), langsung kembalikan.
    - Jika terlihat projected (nilai besar), coba berbagai kombinasi:
      order (easting,northing) & (northing,easting), zone_candidates (46..55), hemi (N/S).
      Pilih kombinasi yang memetakan paling banyak titik ke rentang Indonesia.
    - Jika semua gagal, coba fallback otomatis menebak zone berdasarkan mean easting/northing
      dan/atau coba estimasi zone dari skenario lain.
    """
    if not coords:
        return coords, None, None

    # cek kemungkinan sudah lon/lat
    try:
    ...
    # cek apakah projected-like (nilai besar)
    if not all(abs(float(x)) > 1000 and abs(float(y)) > 1000 for x, y in coords):
        # tidak cukup bukti projected -> kembalikan sebagai float lon/lat (best-effort)
        return [(float(x), float(y)) for x, y in coords], None, None

    # coba kombinasi
    from pyproj import Transformer
    zone_candidates = range(46, 56)  # cakupan aman untuk Indonesia (46..55)
    orders = [("easting_northing", lambda x,y: (x,y)),
              ("northing_easting", lambda x,y: (y,x))]
    ...
    # Fallback otomatis: estimasi zone berdasarkan rata-rata easting (mean_x)
    if best["score"] <= 0:
        try:
            ...
            # Estimasi zone: easting biasanya antara 100000..900000; gunakan perhitungan kasar
            # Konversi mean_x -> probable lon via trial: coba tiap zone, choose zone producing lon in 95..141
            fallback_found = False
            for zone in range(46, 56):
                for hemi in ("N", "S"):
                    ...
                    if cnt_in_try > best["score"]:
                        best.update({
                            "score": cnt_in_try,
                            "transformed": transformed_try,
                            "zone": zone,
                            "hemi": hemi,
                            "order": "auto_fallback_scan"
                        })
                        fallback_found = True
            # if still not found, leave best as is
        except Exception:
            pass

    if best["transformed"] is not None and best["score"] >= 1:
        try:
            st.info(f"Deteksi koordinat PKKPR: order={best['order']}, UTM zone={best['zone']}{best['hemi']}, valid={best['score']}/{len(coords)} titik")
        except Exception:
            pass
        return [(float(lon), float(lat)) for lon, lat in best["transformed"]], best["zone"], best["hemi"]

    # gagal transform dengan baik -> kembalikan as float tanpa zone
    return [(float(x), float(y)) for x, y in coords], None, None
    ...
    if uploaded_pkkpr.name.endswith(".pdf"):
        ...
        with pdfplumber.open(uploaded_pkkpr) as pdf:
            for page in pdf.pages:
                ...
                # ambil teks baris demi baris dan cari pola koordinat serta luas
                ...
                for line in text.split("\n"):
                    ...
                    # cari pola "1 414695.19 90214.00"
        ...
        # dedup dan deteksi proyeksi + zone
        before_count = len(coords_plain)
        coords_unique = deduplicate_coords(coords_plain)
        after_count = len(coords_unique)
        if before_count != after_count:
            st.info(f"Duplicate removed: {before_count} -> {after_count} titik")
        ...
        # simpan info luas dokumen (nilai apa adanya dan satuan)
        ...
        # jika user upload zip shapefile
...
# ======================
if gdf_polygon is not None:
    # tentukan zona UTM PKKPR: pakai detected jika ada, kalau tidak gunakan centroid -> get_utm_info_from_lonlat
...
if gdf_polygon is not None and gdf_tapak is not None:
    ...
    # gunakan utm_epsg_tapak untuk analisis overlay
    gdf_tapak_utm = gdf_tapak.to_crs(epsg=utm_epsg_tapak)
    # gunakan PKKPR utm (calculated previously utm_epsg variable) - if not defined, compute from pkp centroid
...
if gdf_polygon is not None:
    ...
    # tambahkan basemap; jika ctx gagal, jangan crash
    try:
